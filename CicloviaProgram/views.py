...
def editCiclovia(request, ciclovia_id):
    ...
    if ciclovia.arrivals_loaded:
        CicloviaForm = modelform_factory(Ciclovia, fields=('name', 'place', 'start_hour', 'end_hour'
                                                           , 'reference_track', 'reference_hour',
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            return render(request, 'ciclovia/editCiclovia.html',
                          {'form': form, 'formset': formset, 'ciclovia': ciclovia})
    else:
...
def inverseSimulationExecution(request, ciclovia_id):
    ...
    newFlowMeasures = False
    measures = {}
    for track in ciclovia.track_set.all():
        flowHours = []
        for hour in ciclovia.arrivalsproportionperhour_set.all():
            if request.GET.get('track' + str(track.id_track) + 'hour' + str(hour)) != '':
                newFlowMeasures = True
                flowHours.append(request.GET.get('track' + str(track.id_track) + 'hour' + str(hour)))
        flowHours.append(None)
