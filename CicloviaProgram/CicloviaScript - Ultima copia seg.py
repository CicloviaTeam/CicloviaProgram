...
class CicloviaObj:
    ...
    def assignArrivalsToTrack(self):
        print("Entre a asignar los arribos")
        for track in self.tracks:
            arrivalSet = []
            for arrivalAtHour in self.arrivalProportionPerHour:                 
                ...
                print("Se esta multiplicando " + str(arrivalAtHour[1]) + "," + str(track.arrivalProportion) + "," + str(self.referenceArrivalRate) + "," + str(track.distance))
...
class TrackObj:
    ...
    def giveNeighboorInDirection(self, direction):
        ...
        print("Estoy en el track " + str(self.idNum))
        print("Mis tracks son" + str(self.tracksId))
        print("Sus direcciones son" + str(self.tracksDirection))
        for index, direct in enumerate(self.tracksDirection):
            print("La direccion solicitada es " + direction + " la iterada es " + direct[1])
            if(direct[1] == direction):
                print("Entro porque es la misma direccion")
                probabilities.append(self.tracksProb[index])
                ids.append(self.tracksId[index])  
        if(len(ids)==1):
            print("Solo esta el mismo")
    ...
    def updateFlowInTrack(self, time):
        ...
        print("Lista antes de act" + str(self.flowInTrack))
        print("Entro a trayecto con tiempo " + str(time))
        #Changed 60 to 30
        timeInterval = math.floor(time/30)
        print("Time interval " + str(timeInterval))
        print("FlowInTrack" + str(self.flowInTrack))
        self.flowInTrack[int(timeInterval)] += 1  
        print("Lista despues de act" + str(self.flowInTrack))
    ...
    try:       
        ...
        for track in root.findall('track'):                
            ...
            for neighboor in track.findall('neighboor'):
                ...
                #print("ACAAAAAAAAA reviso FromTo")
                #print(str(neighboorsFromToDirec))
                #neighboorDir = 0
...
class SimulationDES:
    ...
    def __init__(self, random_seed, cicloviaId, resultsCompiledId, isValidation):       
        ...
        print("Sim time " + str(self.simTime))
    ...
    def printResults(self, cicloviaId, resultsCompiledId):
        ...
        for track in self.ciclovia.tracks:
            ...
            #print("Para el trayecto " + str(track.idNum) + " los valores son ")               
            #print("Num in track " + str(listNumberInTrack))
            #print("Len num in track" + str(len(listNumberInTrack)))
            #print("Time num in track " + str(listTimeNumberInTrack))  
            #print("Len Time num in track" + str(len(listTimeNumberInTrack)))
       
            #print(str(listNumberInTrack))
            #print(str(listTimeNumberInTrack))
            avgNumberInTrack = 0 
            if(len(listNumberInTrack)==0):
                print("El numero promedio en el trayecto " + str(track.idNum) + " es 0")
    ...
    def assignRoute(self):
        ...
        while(self.timeLeftInSystem>0):
            ...
            if(timeForHalfTrack<self.timeLeftInSystem and (float(self.currentTimeForFlow)+float(timeForHalfTrack)<simTime)):
                ...
                print("Tiempo que se envia es " + str(self.currentTimeForFlow)+ " ; " + str(timeForHalfTrack))
            ...
            if(timeForTrack>self.timeLeftInSystem):
                ...
                if(self.direction == "end"):
                    outIn = "begin"
                print("Direccion donde entro " + str(self.direction))
                print("Direccion donde salgo " + str(outIn))
                nextTrack = self.track.giveNeighboorInDirection(outIn) 
                print("Me retorno " + str(nextTrack))
...
def simulationExecution(cicloviaId, isValidation):
    ...
    for track in range(cicloviaFromDB.num_tracks):
        ...
        print("El num de horas es " + str(timeIntervals))
        listFlowPerHour = []
        for hour in range(timeIntervals):
            listPerHour = []
            listDetailFlowPerTrack.append(listPerHour)
            
    print("La lista para el flujo detallado es " + str(listFlowPerHourTracks))
    ...
    for result in queryFromDB:              
        ...
        #print("La lista info es " + str(listInfoTracks))
        #print("La lista flow es " + str(listFlowTracks))
        for track in infoTracks:	
            #trackId = track.id
            #trackFromDB = SimulationResultsPerTrack.objects.get(id=trackId)                     
            ...
            for flow in queryFlowFromDB:                       
                print("Flow.hour es " + str(flow.hour) + " y el flujo es " + str(flow.flow_hour))
                listFlowPerHourTracks[track.track-1][flow.hour-1].append(flow.flow_hour)   
        print("List flow per hour es " + str(listFlowPerHourTracks))
    ...
    for index, listPerTrack in enumerate(listInfoTracks):
        ...
        print("List Flow Tracks es " + str(listFlowPerHourTracks))
        #for track in listFlowPerHourTracks:
        #print("List per track es " + str(track))
        if(isValidation==True):
            for index2, hourInt in enumerate(listFlowPerHourTracks[index]):
                print("List per hour es " + str(hourInt))
                avg_flow_hourInt = round(numpy.mean(hourInt),3)
                std_flow_hourInt = 0
                if(len(listInfoTracks)>1):
                    std_flow_hourInt = round(numpy.std(hourInt),3)
                print("Avg flow hour es: " + str(avg_flow_hourInt))    
