...
class SimulationDES:
    ...
    #It takes the parameters provided as arguments and stores them in the object
    def __init__(self, random_seed, cicloviaId, isValidation):       
    ...
    def participantArrivals(self):     
        ...
        timeParticipantsSystem = self.weightedValuesSoft(durationValuesArray, durationProbabilitiesArray, (self.ciclovia.maxArrivals*55))
        print("llegoooo")
    ...
    #This definition generates a stream of values from a discrete probability distribution
    def weightedValuesSoft(self, values, probabilities, size):  
        valuesLimits = []
        #print("Values ")
        #print(str(values))
        #print(len(values))
    ...
    def printResults(self, cicloviaId):
        ...
        for track in self.ciclovia.tracks:
            ...
            if(self.isValidation == True): 
                resultsPerTrackDB = resultsDB.simulationresultspertrack_set.create(track = track.idNum, total_arrivals = track.totalArrivals, total_flow = 0,  average_number_track = avgNumberInTrack)
...
def simulationExecution(cicloviaId, isValidation):
    ...
    for i in range(simulationRuns):
        simDES = SimulationDES(seed, cicloviaId, isValidation)
